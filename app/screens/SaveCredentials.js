...
import { StyleSheet, Text, View, Platform, Linking, StatusBar, AsyncStorage, Alert } from 'react-native';
    ...
    if (network === '' || password === '' || buttonSSID === '') {
      ...
      NetworkInfo.getSSID(ssid => {
        if (ssid === `Button ConfigureMe - ${buttonSSID}`) {
          ...
          Alert.alert('Please connect to Button ConfigureMe Network, You are currently connected to: ', ssid)
        }
      });
    }
  }
    ...
    return (
      <View style={styles.container}>
        <View style={{ flex: 2, justifyContent: 'center' }}>
          ...
          <FormLabel inputStyle={{ backgroundColor: 'transparent' }}>Name of your Wifi Network</FormLabel>
          <FormInput
          ...
          />
          <FormLabel inputStyle={{ backgroundColor: 'transparent' }}>Password</FormLabel>
          <FormInput
            underlineColorAndroid="#0C6A9B"
            value={password}
          ...
          />
					<FormLabel inputStyle={{ backgroundColor: 'transparent' }}>Button Network SSID</FormLabel>
          <FormInput
            underlineColorAndroid="#0C6A9B"
            value={buttonSSID}
          ...
          />
          <Text style={{ paddingLeft: 20, paddingTop: 20, color: 'tomato', fontWeight: 'bold', fontSize: 14 }}>{this.state.error && this.state.error}</Text>
        </View>
